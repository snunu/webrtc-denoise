...
Java_com_webrtc_jni_WebRtcUtils_webRtcAgcInit(JNIEnv *env, jclass type, jlong minVolume, jlong maxVolume,
                                              jlong freq,jint mode) {
    int agc = WebRtcAgc_Create(&agcHandle);
    if (agc == 0) {

        int16_t agcMode = kAgcModeUnchanged;
        switch (mode){
            case 0: agcMode = kAgcModeUnchanged;
                break;
            case 1: agcMode = kAgcModeAdaptiveAnalog;
                break;
            case 2: agcMode = kAgcModeAdaptiveDigital;
                break;
            case 3: agcMode = kAgcModeFixedDigital;
                break;
        }
...
JNIEXPORT void JNICALL
Java_com_webrtc_jni_WebRtcUtils_webRtcNsInit(JNIEnv *env, jclass type, jint freq,jint mode) {
